...
public class FontUtil
{
    ...
    public static int guessAWTFontStyle(String name)
    {
        ...
        else
        {
            ...
            {
                if (name.indexOf(AWT_STYLE_ITAL) > 0 ||
    ...
    public static String guessFamily(String name)
    {
        ...
        {
            if ((inx = fam.toLowerCase().lastIndexOf(STYLE_DEMI_ITALIC)) > 0)
            {
                fam = fam.substring(0, inx);
            }
            else
            {
                if ((inx = fam.toLowerCase().lastIndexOf(STYLE_BOLD)) > 0)
                {
                    fam = fam.substring(0, inx);
                }
                else
                {
                    if ((inx = fam.toLowerCase().lastIndexOf(STYLE_ITALIC)) > 0)
                    {
                        fam = fam.substring(0, inx);
                    }
                    else
                    {
                        if ((inx = fam.toLowerCase().lastIndexOf(STYLE_BLACK)) > 0)
                        {
                            fam = fam.substring(0, inx);
                        }
                    }
