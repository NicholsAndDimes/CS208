...
public class Action extends Dictionary
{
    ...
    public static Action buildAction(Library library, HashMap hashMap)
    {
        ...
        if (actionType != null)
        {
            ...
            {
                if (actionType.equals(Action.ACTION_TYPE_GOTO_REMOTE))
                {
                    return new GoToRAction(library, hashMap);
                }
                else
                {
                    if (actionType.equals(Action.ACTION_TYPE_LAUNCH))
                    {
                        return new LaunchAction(library, hashMap);
                    }
                    else
                    {
                        if (actionType.equals(Action.ACTION_TYPE_URI))
                        {
                            return new URIAction(library, hashMap);
                        }
                        else
                        {
                            if (actionType.equals(Action.ACTION_TYPE_RESET_SUBMIT))
                            {
                                return new ResetFormAction(library, hashMap);
                            }
                            else
                            {
                                if (actionType.equals(Action.ACTION_TYPE_SUBMIT_SUBMIT))
                                {
                                    return new SubmitFormAction(library, hashMap);
                                }
                                else
                                {
                                    if (actionType.equals(Action.ACTION_TYPE_NAMED))
                                    {
                                        return new NamedAction(library, hashMap);
                                    }
                                    else
                                    {
                                        if (actionType.equals(Action.ACTION_TYPE_JAVA_SCRIPT))
                                        {
