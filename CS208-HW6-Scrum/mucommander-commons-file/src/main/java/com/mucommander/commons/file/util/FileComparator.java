...
public class FileComparator implements Comparator<AbstractFile>
{
    ...
    private int compareStrings(String s1, String s2, boolean ignoreCase, boolean nullProtection)
    {
        ...
        if (nullProtection)
        {
            ...
            {
                if (s1 != null && s2 == null)    // s2 is null, s1 isn't
                {
                    return 1;
                }
    ...
    public int compare(AbstractFile f1, AbstractFile f2)
    {
        ...
        {
            if (diff < Integer.MIN_VALUE + 1)   // Need that +1 so that the int is not overflowed if ascending order is enabled (i.e. int is negated)
<<< This is the complex bug 'nervous_matsumoto' (instance 4 of 9) >>>
<<< This is the complex bug 'nervous_matsumoto' (instance 5 of 9) >>>
<<< This is the complex bug 'nervous_matsumoto' (instance 6 of 9) >>>
            {
