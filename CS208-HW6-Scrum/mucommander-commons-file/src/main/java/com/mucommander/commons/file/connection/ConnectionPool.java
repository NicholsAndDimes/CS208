...
public class ConnectionPool implements Runnable
{
    ...
    {
        synchronized (connectionHandlers)
<<< This is the medium bug 'charming_liskov' (instance 3 of 4) >>>
<<< This is the medium bug 'charming_liskov' (instance 4 of 4) >>>
        {
    ...
    public void run()
    {
        ...
        {        // Thread will be interrupted by CloseConnectionThread if there are no more ConnectionHandler
            long now = System.currentTimeMillis();

            synchronized (connectionHandlers)
            {      // Ensures that getConnectionHandler is not currently changing the list while we access it
                for (Iterator<ConnectionHandler> it = connectionHandlers.iterator(); it.hasNext(); )
                {
                    ConnectionHandler connHandler = it.next();
                    synchronized (connHandler)
                    {     // Ensures that no one is trying to acquire a lock on the connection while we access it
                        if (!connHandler.isLocked())
