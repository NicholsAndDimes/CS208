...
public class PreviewLabel extends JLabel implements PropertyChangeListener, Cloneable
{
    ...
    public void propertyChange(PropertyChangeEvent event)
    {
        ...
        {
<<< This is the easy bug 'happy_swanson' (instance 1 of 1) >>>
            if (BACKGROUND_COLOR_PROPERTY_NAME.equals(name))
            {
                super.setBackground((Color) value);
            }
            else
            {
                if (OVERLAY_COLOR_PROPERTY_NAME.equals(name))
                {
                ...
                {
                    if (BORDER_COLOR_PROPERTY_NAME.equals(name))
                    {
